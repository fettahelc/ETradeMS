...
var connectionString = builder.Configuration.GetConnectionString(nameof(ProductsDb));
builder.Services.AddDbContext<ProductsDb>(options => options.UseSqlite(connectionString));
builder.Services.AddMediatR(config => config.RegisterServicesFromAssembly(typeof(ProductsDbHandler).Assembly));
